...
EE_LOW = (0.2, -0.2, 0.4)
EE_HIGH = (0.5, 0.4, 0.6)
ANGLE_LOW = (-np.pi/4, -np.pi/4, -np.pi/2)
ANGLE_HIGH = (np.pi/4, np.pi/4, np.pi/2)
...
DIR = osp.join('/home/gwthomas/workspace/gps/experiments/pr2_cad/augmentation', randstr(8))
...
def iteration(itr, cfg, agent, algorithm):
    conditions = cfg['common']['conditions']
