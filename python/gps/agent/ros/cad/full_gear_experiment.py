...
class FullGearExperiment(AgentCAD):
    ...
    def setup(self):
        self.move_head(6, 0, -3.5) # Move head so we can see what we need to see
    ...
    def configure_scene(self):
        ...
        # Add the position of everything

        sizes = {'compound_gear': (0.025, 0.025, 0.025), 'base_plate': (0.025, 0.025, 0.025)}

        for name in ['compound_gear', 'base_plate']:
    ...
    def reset_piece(self):
        ...
        #pose = Pose(Point(0.778409, 0.274577, 0.586252), quat)
        pose = Pose(Point(0.935045, -0.046478, 0.593991), quat)
        self.set_pose('compound_gear', pose)
    ...
    def grasp_prep(self):
        ...
        time.sleep(3)
        #gear_pose, euler = self.pose_from_AR('compound_gear') # Gear has AR marker 1 on it        
        #gear_pose, euler = self.pose_from_AR('compound_gear') # Gear has AR marker 1 on it
        gear_pose = self.get_pose('compound_gear')
        pos = gear_pose.position
        # Move to the position depending on where the AR tag was
        #self.move_to(pos.x - 0.238, pos.y, pos.z + 0.008, 1.57, euler[1], euler[2])
        self.move_to(pos.x - 0.238, pos.y, pos.z + 0.008, 1.57, 0, 0)
    ...
    def error_ar(self, obj_name):
        pose, euler = self.pose_from_AR(obj_name)
        return np.array(listify(pose.position)) - np.array(listify(self.get_pose(obj_name).position))

    def finish_prep(self):
        # Get the previous orientation of the gear
        self.scene.remove_world_object('compound_gear') # Remove it 

        #pose, euler = self.pose_from_AR('compound_gear') # Get the pose now that it has changed
        #pose, euler = self.pose_from_AR('compound_gear') # Get the pose now that it has changed
        pose = self.get_pose('compound_gear')
        ...
        self.attach('compound_gear', touch_links=['l_gripper_l_finger_tip_link', 'l_gripper_r_finger_tip_link'])# ,'l_gripper_l_finger_link'])
    ...
    def move_to_pose_and_ori(self, position, orientation):
        init_plan = self.plan_end_effector(listify(position), listify(orientation))
    ...
    # Made to change goals depending on the weird rotation of the gear lololol
    def offset_rotation(self):
        self.use_controller('MoveIt')
        self.move_to(0.555, 0.3, 0.7, 1.57, 0, 0) # Move the gear up
        time.sleep(2) # Wait three seconds
        gear_pose, euler = self.pose_from_AR('compound_gear') # Get the gear pose from AR
        gear_pose, euler = self.pose_from_AR('compound_gear') # Get the gear pose from AR
        print(gear_pose)
        print(euler)
        euler1 = -1 * euler[1]
        ...
        self.move_to(0.555, 0.3, 0.7, 1.57, euler1, euler2) # Rotate the gear now
        time.sleep(3)
        print('getting gear_pose')
        gear_pose, euler = self.pose_from_AR('compound_gear') # Now get the full gear pose
        gear_pose, euler = self.pose_from_AR('compound_gear') # Now get the full gear pose
        ...
        base_pose, euler = self.pose_from_AR('base_plate') # Get the position of the box by AR tag
        base_pose, euler = self.pose_from_AR('base_plate')# Get the position of the box by AR tag

        # This depends on where the gear is supposed to go and whatever
        new_pos[0] = robot_pos.x + (base_pose.position.x - gear_pose.position.x)
        new_pos[1] = robot_pos.y + (base_pose.position.y - gear_pose.position.y) 
        new_pos[2] = robot_pos.z + (base_pose.position.z + 0.038 - gear_pose.position.z)

        # For as many conditions there are 
        for i in range(self._hyperparams['conditions']):
            the_tuple = self._hyperparams['targets'][i]['orientation']
            new_thing = [the_tuple[0], euler1, euler2]
        ...
        # Add this gear to see where it is in rviz
        #self.add_object('xtra_compound_gear', position=[base_pose.position.x, base_pose.position.y, base_pose.position.z + 0.038], 
        #    orientation=[0,0,0,0],
        #    size=(0.025, 0.025, 0.025), 
        #       filename=self._hyperparams['compound_gear'])

    def remove_xtra_gear(self):
        self.scene.remove_world_object('xtra_compound_gear')
    ...
    def redo_offset(self):
        ...
        # Originally 0.038 but i wanna make it easier
        # This depends on where the gear is supposed to go and whatever
        new_pos[0] = robot_pos.x + (base_pose.position.x - gear_pose.position.x)
        new_pos[1] = robot_pos.y + (base_pose.position.y - gear_pose.position.y) 
        new_pos[2] = robot_pos.z + (base_pose.position.z + 0.038 - gear_pose.position.z)
    ...
    def reset(self, condition):
        super(FullGearExperiment, self).reset(condition) # Call the super method
